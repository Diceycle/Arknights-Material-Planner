...
def splitScreenshot(screenshot):
    boxes = []
    for x in range(BOXES_ON_SCREEN[0]):
        for y in range(BOXES_ON_SCREEN[1]):
            posX = FIRST_MATERIAL_CENTER[0] + x * MATERIAL_DISTANCE[0]
            posY = FIRST_MATERIAL_CENTER[1] + y * MATERIAL_DISTANCE[1]
...
class DepotParser:
    ...
    def startParsing(self):
        ...
        self.boxes = splitScreenshot(screenshot)
        return None
    ...
    def parseNext(self):
        if self.boxIndex >= len(self.boxes):
            if self.handler is not None and self.scrollNotified:
                ...
                self.boxes = splitScreenshot(takeScreenshot(self.handler))
                self.boxIndex = 0
    ...
    def parseMaterialAmount(self):
        material = DEPOT_ORDER[self.materialIndex]
        box = self.boxes[self.boxIndex]

        confidence = matchMasked(box, MATERIALS[material])
        ...
        if self.expectedResult is not None:
            if self.expectedResult[self.materialIndex] != amount:
                print("!!! Material at Position {:>2}: {:>16} x{:<4} but should be {}".format(
                    self.boxIndex, material, amount, self.expectedResult[self.materialIndex]))
            else:
                print("    Material at Position {:>2}: {:>16} x{:<4}".format(self.boxIndex, material, amount))
