...
| `offlineMode`                | Boolean         | `false`                                 | Whether to start in offline mode. In this mode no Web-Requests are made but that could cause data to be outdated or images to be missing. Also note that the tool needs to start at least one time without offline mode to download initial data. It could also be necessary to turn off offline mode after an update. |
| `webRequestTimeout`          | Number          | `1`                                     | Time to wait for a response to Web-Requests in seconds. Raise this parameter if you see consistent timeout messages in the log.                                                                                                                                                                                        |
...
| `resizeArknights`            | Boolean         | `false`                                 | Whether to resize the Arknights window before starting a depot scan. Recommended if Arknights is run in a smaller than 1600x900 window, as number readings might get inaccurate at this scale.                                                                                                                         |
