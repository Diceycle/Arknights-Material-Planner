...
| `arknightsInputWindowName`   | String          | `"plrNativeInputWindowClass"`                | The name of the **child window class** that accepts input. This is an **expert setting**. Refer to `FindArknightsWindow.exe` to try and figure out what to set here. Can also be set to `null` to use the top-level window. |
